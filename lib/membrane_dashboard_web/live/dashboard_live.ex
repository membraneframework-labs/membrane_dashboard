defmodule Membrane.DashboardWeb.DashboardLive do
  ...
  @timestamps_regex Regex.compile!("^from=([0-9]+)&to=([0-9]+)$")
  ...
  @impl true
  def handle_event("validate-timestamps", %{"timestamps" => timestamps}, socket) do
    with {:ok, _} <- parse_timestamps(timestamps) do
      {:noreply, assign(socket, timestamps: timestamps)}
    else
      {:error, reason} ->
  ...
  def handle_event("refresh", _values, socket) do
    with {:ok, {from, to}} <- parse_timestamps(socket.assigns.timestamps),
  ...
  defp parse_timestamps(timestamps) do
    with [_, from, to] <- Regex.run(@timestamps_regex, timestamps) do
