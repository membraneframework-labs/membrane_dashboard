defmodule Membrane.Dashboard.Charts do
  ...
  def query(methods, time_from, time_to, accuracy) do
    charts_data =
      methods
      |> Enum.map(fn method -> one_chart_query(method, time_from, time_to, accuracy) end)
  ...
  defp create_interval(from, to, accuracy) do
    [from, to] = [apply_accuracy(from, accuracy), apply_accuracy(to, accuracy)]
