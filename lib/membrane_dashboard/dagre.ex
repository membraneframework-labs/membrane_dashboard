defmodule Membrane.Dashboard.Dagre do
  ...
  def query_dagre(time_from, time_to) do
    result = "SELECT parent_path, l.from, l.to, pad_from, pad_to FROM links l WHERE time BETWEEN '#{parse_time(time_from)}' AND '#{parse_time(time_to)}'"
      |> Repo.query()
  ...
  defp format_rows(rows) do
    rows |> Enum.map(fn [parent_path, from, to, pad_from, pad_to] ->
      %{
